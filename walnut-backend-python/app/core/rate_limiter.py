...
Key 组成：``[根前缀:]global:rate_limit:{请求URI}[:{ip}|:{instance}]:{key}``
（根前缀即 settings.REDIS_KEY_PREFIX，由 full_key 统一叠加）。
...
from app.core.redis_crud import RedisUtils, full_key
...
class RateLimiter:
    ...
    async def __call__(self, request: Request, redis=Depends(redis_getter)) -> None:
        ...
        r = RedisUtils(redis)
        try:
            current = await r.redis.eval(_RATE_LIMIT_LUA, 1, full_key(combine_key), self.time)
