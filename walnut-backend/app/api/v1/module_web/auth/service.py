...
- ``user_session:<session_id>``：会话 JSON（user_id/user_name/nickname/dept_id/user_status/
  menu_permission/role_permission/menu_ids），TTL=client.timeout；
...
class AuthService:
    ...
    async def do_login(self, user, session_info: dict, client) -> dict:
        ...
        # JWT 固定过期取 client.timeout；滑动过期由 Redis TTL（active_timeout）驱动续期
        jwt_expire = timeout if not settings.TOKEN_SLIDING_EXPIRE else max(timeout, active_timeout)
        ...
        await self.redis_util.set(f"user_session:{session_id}", json.dumps(session_info, ensure_ascii=False), expire=timeout)
        await self.redis_util.set(f"access_token:{session_id}", token, expire=timeout)

        # 在线用户信息（写入 online_tokens:<token> 键）
        online_dto = await self._build_online_dto(user, session_info, client, token)
        await self.redis_util.set(f"{CacheNames.ONLINE_TOKEN_KEY}{token}", json.dumps(online_dto, ensure_ascii=False), expire=timeout)
