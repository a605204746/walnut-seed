...
async def _authenticate(request: Request, token: str, redis: Redis) -> AuthSchema:
    ...
    # 滑动过期模式下跳过 JWT exp 校验，由 Redis 会话 TTL 决定实际有效期
    payload: JWTPayloadSchema = decode_access_token(token, verify_exp=not settings.TOKEN_SLIDING_EXPIRE)
    ...
    # 滑动过期续期：以鉴权实际读取的 user_session 键 TTL 为准，
    # 过半程时同时续期会话键与令牌键，保持两者生命周期一致
    if settings.TOKEN_SLIDING_EXPIRE:
        ...
        if 0 < ttl < expire_seconds // 2:
            await RedisUtils(redis).expire(session_key, expire_seconds)
            await RedisUtils(redis).expire(access_key, expire_seconds)
