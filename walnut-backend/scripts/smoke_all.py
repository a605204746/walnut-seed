"""walnut-backend 全端点冒烟验收。

覆盖：认证全流程 + module_system/module_web/monitor 全部核心端点。
用法（walnut-backend 目录）：PYTHONPATH=. ENVIRONMENT=dev .venv/Scripts/python scripts/smoke_all.py
"""
...
async def main():
    ...
    async with httpx.AsyncClient(base_url=BASE, timeout=30) as c:
        ...
        ans = rr.get("global:captcha_codes:" + j["data"]["uuid"])
        ...
        ans = rr.get("global:captcha_codes:" + d["uuid"])
