...
> 基础命令与执行时机已见于根目录 [README「数据库迁移」](../../README.md#数据库迁移alembic)，本文聚焦机制原理与实战细节。

本项目 Schema 的唯一事实来源是 `walnut-backend/app/alembic/versions/` 下的迁移脚本，**没有**启动时 `create_all`。这篇教程讲清楚：迁移是怎么跑起来的、日常工作流怎么走、出了问题怎么救。
