...
本文只说明 Java 后端编排。Python 后端请阅读 [Python Docker 部署](../python/10-Docker部署.md)；两份编排共用中间件定义但互斥运行。
...
| 停止并清理 Docker 具名卷 | `docker compose -f docker/docker-compose.java.yml down -v`（不会删除 `docker/volumes/logs`） |
