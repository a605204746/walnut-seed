...
本清单适用于 Java 编排，完整部署流程见 [Java Docker 部署](./10-Docker部署.md)。迁移、容器和配置示例均以 Java/Spring Boot/Flyway 为准。
...
| RSA 密钥黑名单 | prod 配置了 RuoYi 生态公开的出厂密钥 | 重新生成并配置生产密钥 |
| Flyway 迁移 | Spring Boot 启动时迁移失败 | 修复迁移脚本或补充后续版本 |
...
- [ ] **接口加解密 RSA 密钥**：生产生成并配置**两对**密钥，按 [Java 接口加解密配置](./06-接口加解密.md) 的配对关系分别填前后端；前端需重新构建镜像（密钥构建期注入）
...
- [ ] **迁移脚本随代码提交**：Flyway 在 Spring Boot 启动时自动执行，确认本次发布的 `db/migration` 脚本已入库
- [ ] **备份**：MySQL 数据在 `docker/volumes/mysql`，Java 日志在 `docker/volumes/logs`，分别纳入备份/清理策略
- [ ] **只读核对**：首次启动后 `docker logs walnut-backend-java` 应看到 Flyway 迁移完成且无异常堆栈
...
- [ ] **日志级别**：生产使用 INFO，关闭 debug 和 SQL 全量输出；Docker 日志文件位于 `docker/volumes/logs/`，文件名以 `walnut-seed-java-` 开头
...
- [ ] **接口加解密**：确认启动日志没有加解密停用告警（有则说明密钥未配好，见 [Java 加解密教程](./06-接口加解密.md)）

## 6. 上线后验证

1. `docker compose -f docker/docker-compose.java.yml ps` —— 5 个服务全部 healthy
