...
cd walnut-backend-python && uv sync && uv run main.py run --env dev   # 后端能正常启动
cd walnut-frontend && pnpm install && pnpm dev:antd            # 前端能登录
```

## 1. 创建后端模块包

业务模块统一放在 `walnut-backend-python/app/api/v1/module_*/`。新建包目录：

```
walnut-backend-python/app/api/v1/module_blog/
...
dev 环境启动时也会按 `DATABASE_AUTO_MIGRATE=True` 自动迁移，但显式 upgrade 能第一时间暴露脚本问题。迁移细节与常见坑见 [Alembic 迁移实战](./09-数据库迁移-Alembic.md)。
...
- 数据权限（行级过滤）如何接入，见 [权限配置 · 第 4 节](./05-权限配置.md#4-数据权限行级)
