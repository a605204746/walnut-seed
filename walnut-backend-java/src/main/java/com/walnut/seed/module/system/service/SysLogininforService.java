package com.walnut.seed.module.system.service;
import com.walnut.seed.common.core.domain.PageResult;

import com.walnut.seed.module.system.domain.req.SysLogininforReq;
import com.walnut.seed.module.system.domain.resp.SysLogininforResp;

import java.util.List;

/**
 * 系统访问日志情况信息 服务层
 *
 * @author deepin_sir
 */
public interface SysLogininforService {

    /**
     * 分页查询登录日志列表
     *
     * @param req 查询条件
     * @return 登录日志分页列表
     */
    PageResult<SysLogininforResp> selectPageLogininforList(SysLogininforReq req);

    /**
     * 新增系统登录日志
     *
     * @param req 访问日志对象
     */
    void insertLogininfor(SysLogininforReq req);

    /**
     * 查询系统登录日志集合
     *
     * @param req 访问日志对象
     * @return 登录记录集合
     */
    List<SysLogininforResp> selectLogininforList(SysLogininforReq req);

    /**
     * 批量删除系统登录日志
     *
     * @param infoIds 需要删除的登录日志ID
     * @return 结果
     */
    int deleteLogininforByIds(Long[] infoIds);

    /**
     * 清空系统登录日志
     */
    void cleanLogininfor();
}
