 ...
 * @author deepin_sir
 */
public interface SysDeptMapper extends BaseMapperPlus<SysDept, SysDeptResp> {
    ...
    default String buildDeptByRoleSql(Long roleId) {
        return """
                    select srd.dept_id from sys_role_dept srd
                        left join sys_role sr on sr.id = srd.role_id
    ...
    default String buildParentDeptByRoleSql(Long roleId) {
        return """
                    select parent_id from sys_dept where id in (
                        select srd.dept_id from sys_role_dept srd
                            left join sys_role sr on sr.id = srd.role_id
                            where srd.role_id = %d and sr.status = '0'
                    )
    ...
    default List<SysDeptResp> selectDeptList(Wrapper<SysDept> queryWrapper) {
    ...
    default Page<SysDeptResp> selectPageDeptList(Page<SysDept> page, Wrapper<SysDept> queryWrapper) {
    ...
    default long countDeptById(Long deptId) {
        return this.selectCount(new LambdaQueryWrapper<SysDept>().eq(SysDept::getId, deptId));
    }
    ...
    default List<Long> selectDeptListByRoleId(Long roleId, boolean deptCheckStrictly) {
        ...
        wrapper.select(SysDept::getId)
                .inSql(SysDept::getId, this.buildDeptByRoleSql(roleId))
                .orderByAsc(SysDept::getParentId)
                .orderByAsc(SysDept::getOrderNum);
        if (deptCheckStrictly) {
            wrapper.notInSql(SysDept::getId, this.buildParentDeptByRoleSql(roleId));
        }
