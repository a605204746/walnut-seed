package com.walnut.seed.common.mybatis.handler;
...
public class MybatisExceptionHandler {
    ...
    public ApiResponse<Void> handleDuplicateKeyException(DuplicateKeyException e, HttpServletRequest request) {
        ...
        return ApiResponse.fail(HttpStatus.HTTP_CONFLICT, "数据库中已存在该记录，请联系管理员确认");
    }
    ...
    public ApiResponse<Void> handleCannotFindDataSourceException(MyBatisSystemException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        Throwable root = getRootCause(e);
        if (root instanceof NotLoginException) {
            log.error("请求地址'{}',认证失败'{}',无法访问系统资源", requestURI, root.getMessage());
            return ApiResponse.fail(HttpStatus.HTTP_UNAUTHORIZED, "认证失败，无法访问系统资源");
        }
        if (root instanceof CannotFindDataSourceException) {
            log.error("请求地址'{}', 未找到数据源", requestURI);
            return ApiResponse.fail(HttpStatus.HTTP_INTERNAL_ERROR, "未找到数据源，请联系管理员确认");
        }
        log.error("请求地址'{}', Mybatis系统异常", requestURI, e);
        return ApiResponse.fail(HttpStatus.HTTP_INTERNAL_ERROR, e.getMessage());
    }
