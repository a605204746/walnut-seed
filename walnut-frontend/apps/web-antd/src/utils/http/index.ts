  ...
  responded: {
    onSuccess: async (response, instance) => {
      ...
      if (hasSuccess) {
        ...
        // 非分页: data 为业务对象，直接返回
        // 分页: data 为 { rows, total }，直接返回
        // data 为 null/undefined 时返回剩余参数作为兜底
        if (data !== undefined) {
          return data;
        }
        // 没有data 将其他参数包装为data
...
/**
 * 统一请求对象，替代对 alovaInstance 的直接 monkey-patching。
 * 配置类型 AxiosRequestConfig 通过 types/alova.d.ts 扩展了业务字段。
 */
export const request = {
  ...
  getWithMsg: <T>(url: string, config?: AxiosRequestConfig) =>
    alovaInstance.Get<T>(url, { ...config, successMessageMode: 'message' }),
  postWithMsg: <T>(url: string, data?: any, config?: AxiosRequestConfig) =>
    alovaInstance.Post<T>(url, data, { ...config, successMessageMode: 'message' }),
  putWithMsg: <T>(url: string, data?: any, config?: AxiosRequestConfig) =>
    alovaInstance.Put<T>(url, data, { ...config, successMessageMode: 'message' }),
  deleteWithMsg: <T>(url: string, data?: any, config?: AxiosRequestConfig) =>
    alovaInstance.Delete<T>(url, data, { ...config, successMessageMode: 'message' }),
};
