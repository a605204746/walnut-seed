  ...
  proxyConfig: {
    ajax: {
      query: async (_, formValues = {}) => {
        ...
        // 手动转为树结构
        const treeData = listToTree(resp, { id: 'id', pid: 'parentId' });
        // 添加hasChildren字段
...
async function handleDelete(row: Menu) {
  if (cascadingDeletion.value) {
    // 级联删除
    const menuAndChildren: Menu[] = treeToList([row], { id: 'id' });
    await menuCascadeRemove(menuAndChildren.map((item) => item.id));
  } else {
    // 单删除
...
function removeConfirmTitle(row: Menu) {
  ...
  const menuAndChildren = treeToList([row], { id: 'id' });
