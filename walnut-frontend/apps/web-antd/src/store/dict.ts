 ...
 * 本质上是对DictData的拓展
 */
export interface DictOption extends Omit<DictData, 'id'> {
  disabled?: boolean;
  label: string;
  value: number | string;
}
...
export function dictToOptions(
  ...
  // 显式剔除数字主键 id：antdv-next 的 RadioGroup 会把选项的 id 透传给
  // ARadio 的 id prop（期望 string），数字主键会触发 prop 类型告警。
  // 其余字段（dictValue/listClass/cssClass 等）保留，字典展示组件仍在使用。
  return data.map(({ id: _id, ...item }) => ({
    ...item,
