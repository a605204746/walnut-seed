...
export function loginInfoList(params?: PageQuery) {
  return request.get<PageResult<LoginLog>>(Api.loginInfoList, { params });
}
...
export function loginInfoRemove(infoIds: IDS) {
  return request.deleteWithMsg<void>(`${Api.root}/${infoIds}`);
}
...
export function userUnlock(username: string) {
  return request.get<void>(`${Api.userUnlock}/${username}`, {
...
export function loginInfoClean() {
  return request.deleteWithMsg<void>(Api.loginInfoClean);
}
