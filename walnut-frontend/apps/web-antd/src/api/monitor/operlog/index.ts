...
export function operLogList(params?: PageQuery) {
  return request.get<PageResult<OperationLog>>(Api.operLogList, {
    params,
  });
}
...
export function operLogDelete(operIds: IDS) {
  return request.deleteWithMsg<void>(`${Api.root}/${operIds}`);
}
...
export function operLogClean() {
  return request.deleteWithMsg<void>(Api.operLogClean);
}
