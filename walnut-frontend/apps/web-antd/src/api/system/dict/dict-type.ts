...
export function dictTypeList(params?: PageQuery) {
  return request.get<PageResult<DictType>>(Api.dictTypeList, { params });
}
...
export function dictTypeRemove(dictIds: IDS) {
  return request.deleteWithMsg<void>(`${Api.root}/${dictIds}`);
}
...
export function refreshDictTypeCache() {
  return request.deleteWithMsg<void>(Api.dictTypeRefreshCache);
}
...
export function dictTypeInfo(dictId: ID) {
  return request.get<DictType>(`${Api.root}/${dictId}`);
}
...
export function dictOptionSelectList() {
  return request.get<DictType[]>(Api.dictOptionSelectList);
}
