...
export function clientList(params?: PageQuery) {
  return request.get<PageResult<Client>>(Api.clientList, { params });
}
...
export function clientInfo(id: ID) {
  return request.get<Client>(`${Api.root}/${id}`);
}
...
export function clientChangeStatus(data: any) {
  ...
  return request.putWithMsg<void>(Api.clientChangeStatus, requestData);
}
...
export function clientRemove(ids: IDS) {
  return request.deleteWithMsg<void>(`${Api.root}/${ids}`);
}
