...
export function userList(params?: PageQuery) {
  return request.get<PageResult<User>>(Api.userList, { params });
}
...
export function findUserInfo(userId?: ID) {
  ...
  return request.get<UserInfoResponse>(url);
}
...
export function userStatusChange(data: Partial<User>) {
  ...
  return request.putWithMsg<void>(Api.userStatusChange, requestData);
}
...
export function userRemove(userIds: IDS) {
  return request.deleteWithMsg<void>(`${Api.root}/${userIds}`);
}
...
export function userResetPassword(data: ResetPwdParam) {
  return request.putWithMsg<void>(Api.userResetPassword, data, {
    encrypt: true,
  });
}
...
export function getUserAuthRole(userId: ID) {
  return request.get(`${Api.userAuthRole}/${userId}`);
}
...
export function userAuthRoleUpdate(userId: ID, roleIds: number[]) {
  return request.putWithMsg(Api.userAuthRole, { roleIds, userId });
}
...
export function getDeptTree() {
  return request.get<DeptTree[]>(Api.deptTree);
}
...
export function listUserByDeptId(deptId: ID) {
  return request.get<User[]>(`${Api.listDeptUsers}/${deptId}`);
}
