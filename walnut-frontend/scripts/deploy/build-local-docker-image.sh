...
function build_image() {
    # build docker (use absolute paths to be independent of current working directory)
    # nginx.conf 在仓库 docker/config/ 目录，经 --build-context 注入（见 Dockerfile COPY --from=deploy）
    docker build "${SCRIPT_DIR}/../.." -f "${SCRIPT_DIR}/Dockerfile" --build-context deploy="${SCRIPT_DIR}/../../../docker" -t ${IMAGE_NAME} || ERROR="build_image failed"
}
...
function log_message() {
    ...
    then
        ...
        echo ""
        echo "docker run -d -p 8010:80 --name ${IMAGE_NAME} ${IMAGE_NAME}"
    fi
}
